...
void main_user()
{
	uint32_t frequency = 1;	// Desired Frequency in Hz
	...
	while(1)
	{
		...
		// Change Frequency
		uint32_t new_arr = (72000000 / ((htim3.Init.Prescaler + 1) * frequency)) - 1;
		__HAL_TIM_SET_AUTORELOAD(&htim3, new_arr);
		frequency *= 2;	// Double the frequency for the next change
		if (frequency > 16) frequency = 1;	// Reset to 1 Hz after reaching 16 Hz
		// Do nothing ...
	}
}
